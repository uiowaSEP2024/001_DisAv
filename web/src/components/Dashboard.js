...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3002/user/get-by-username?username=${user.username}`)
      .then(res => {
  ...
  function addSite(text, listType) {
    const updatedSites = listType === 'blacklist' ? [...trackedSites, text] : [...whitelistedSites, text];
  ...
  function removeSite(index, listType) {
    const newSites = listType === 'blacklist' ? trackedSites.filter((_, i) => i !== index) : whitelistedSites.filter((_, i) => i !== index);
    axios.put('http://localhost:3002/user/update-all-preferences', {
      username: user.username,
